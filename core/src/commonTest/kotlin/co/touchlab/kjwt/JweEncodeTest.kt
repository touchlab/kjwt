...
@OptIn(DelicateKJWTApi::class) class JweEncodeTest :
    FunSpec({
        ...
        context("structure / header checks") {
            ...
            test("encrypt Dir A256GCM header contains alg and enc") {
                ...
                assertTrue(
                    headerJson.contains("\"enc\":\"A256GCM\""),
                    "Header must contain enc=A256GCM, got: $headerJson"
                )
            }
            ...
            test("encrypt RsaOaep A256GCM with kid") {
                ...
                val headerJson = decodeTokenHeader(token)
                assertTrue(
                    headerJson.contains("\"kid\":\"rsa-enc-key-id\""),
                    "Header must contain kid, got: $headerJson"
                )
            }
        }
