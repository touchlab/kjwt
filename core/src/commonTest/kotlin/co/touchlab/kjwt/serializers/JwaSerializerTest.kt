    ...
    FunSpec({
        ...
        context("serialize") {
            ...
            test("serialize HS256 encodes id") {
                val result = Jwt.defaultJsonParser.encodeToString(JwaSerializer, SigningAlgorithm.HS256)
                assertEquals("\"HS256\"", result)
            }

            test("serialize RS256 encodes id") {
                val result = Jwt.defaultJsonParser.encodeToString(JwaSerializer, SigningAlgorithm.RS256)
                assertEquals("\"RS256\"", result)
            }

            test("serialize ES256 encodes id") {
                val result = Jwt.defaultJsonParser.encodeToString(JwaSerializer, SigningAlgorithm.ES256)
                assertEquals("\"ES256\"", result)
            }

            test("serialize none encodes id") {
                val result = Jwt.defaultJsonParser.encodeToString(JwaSerializer, SigningAlgorithm.None)
                assertEquals("\"none\"", result)
            }

            test("serialize RsaOaep encodes id") {
                val result = Jwt.defaultJsonParser.encodeToString(JwaSerializer, EncryptionAlgorithm.RsaOaep)
                assertEquals("\"RSA-OAEP\"", result)
            }

            test("serialize RsaOaep256 encodes id") {
                val result = Jwt.defaultJsonParser.encodeToString(JwaSerializer, EncryptionAlgorithm.RsaOaep256)
                assertEquals("\"RSA-OAEP-256\"", result)
            }

            test("serialize Dir encodes id") {
                val result = Jwt.defaultJsonParser.encodeToString(JwaSerializer, EncryptionAlgorithm.Dir)
            ...
            test("serialize all signing algorithms use id as value") {
                ...
                val actualIds =
                    SigningAlgorithm.entries.map { alg ->
                        Jwt.defaultJsonParser.encodeToString(JwaSerializer, alg).removeSurrounding("\"")
                    }
            ...
            test("serialize all encryption algorithms use id as value") {
                ...
                val actualIds =
                    EncryptionAlgorithm.entries.map { alg ->
                        Jwt.defaultJsonParser.encodeToString(JwaSerializer, alg).removeSurrounding("\"")
                    }
                assertEquals(expectedIds, actualIds)
            }
        ...
        context("deserialize") {
            ...
            test("deserialize case mismatch throws IllegalArgumentException") {
                ...
                assertFailsWith<IllegalArgumentException> {
                    Jwt.defaultJsonParser.decodeFromString(JwaSerializer, "\"hs256\"")
                }
            }
        }
        ...
        context("round-trip") {
            ...
            test("round trip all signing algorithms preserves identity") {
                for (alg in SigningAlgorithm.entries) {
                    val encoded = Jwt.defaultJsonParser.encodeToString(JwaSerializer, alg)
                    val decoded = Jwt.defaultJsonParser.decodeFromString(JwaSerializer, encoded)
