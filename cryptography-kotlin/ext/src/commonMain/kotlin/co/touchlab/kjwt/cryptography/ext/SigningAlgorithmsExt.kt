...
import co.touchlab.kjwt.model.algorithm.SigningAlgorithm.EdDSABased
...
/**
 * Generates a new EdDSA key pair for use with this algorithm.
 *
 * The returned [SigningKey] is a [SigningKey.SigningKeyPair] containing both the public and
 * private key, usable for signing and verification.
 *
 * @param keyId optional key ID to associate with the generated key pair. Defaults to `null`.
 * @param cryptographyProvider the provider used to perform key generation.
 * @return a [SigningKey] wrapping the generated [EdDSA] key pair.
 */
@OptIn(DelicateKJWTApi::class)
public suspend fun EdDSABased.newKey(
    keyId: String? = null,
    cryptographyProvider: CryptographyProvider = CryptographyProvider.Default,
): SigningKey.SigningKeyPair {
    val keyPair =
...
/**
 * Decodes an EdDSA public key from a [ByteArray] for use with this algorithm.
 *
 * The returned [SigningKey] is a [SigningKey.VerifyOnlyKey] that can verify signatures but
 * cannot produce them.
 *
 * @param key the public key material to decode.
 * @param keyId optional key ID to associate with the decoded key. Defaults to `null`.
 * @param format the format in which [key] is encoded. Defaults to [EdDSA.PublicKey.Format.RAW].
 * @param cryptographyProvider the provider used to perform key decoding.
 * @return a [SigningKey] wrapping the decoded [EdDSA.PublicKey].
 */
@OptIn(DelicateKJWTApi::class)
public suspend fun EdDSABased.parsePublicKey(
    key: ByteArray,
    keyId: String? = null,
    format: EdDSA.PublicKey.Format = EdDSA.PublicKey.Format.RAW,
    cryptographyProvider: CryptographyProvider = CryptographyProvider.Default,
): SigningKey.VerifyOnlyKey {
    val parsedKey =
        cryptographyProvider
...
/**
 * Decodes an EdDSA private key from a [ByteArray] for use with this algorithm.
 *
 * The returned [SigningKey] is a [SigningKey.SigningOnlyKey] that can produce signatures but
 * cannot verify them.
 *
 * @param key the private key material to decode.
 * @param keyId optional key ID to associate with the decoded key. Defaults to `null`.
 * @param format the format in which [key] is encoded. Defaults to [EdDSA.PrivateKey.Format.RAW].
 * @param cryptographyProvider the provider used to perform key decoding.
 * @return a [SigningKey] wrapping the decoded [EdDSA.PrivateKey].
 */
@OptIn(DelicateKJWTApi::class)
public suspend fun EdDSABased.parsePrivateKey(
    key: ByteArray,
    keyId: String? = null,
    format: EdDSA.PrivateKey.Format = EdDSA.PrivateKey.Format.RAW,
    cryptographyProvider: CryptographyProvider = CryptographyProvider.Default,
): SigningKey.SigningOnlyKey {
    val parsedKey =
        cryptographyProvider
...
/**
 * Decodes an EdDSA key pair from separate public and private key [ByteArray]s.
 *
 * The returned [SigningKey] is a [SigningKey.SigningKeyPair] containing both keys, usable for
 * signing and verification.
 *
 ...
 * @param privateKey the private key material to decode.
 * @param keyId optional key ID to associate with the decoded key pair. Defaults to `null`.
 * @param publicKeyFormat the format in which [publicKey] is encoded. Defaults to [EdDSA.PublicKey.Format.RAW].
 * @param privateKeyFormat the format in which [privateKey] is encoded. Defaults to [EdDSA.PrivateKey.Format.RAW].
 * @param cryptographyProvider the provider used to perform key decoding.
 * @return a [SigningKey] wrapping the decoded [EdDSA] key pair.
 */
@OptIn(DelicateKJWTApi::class)
public suspend fun EdDSABased.parseKeyPair(
    publicKey: ByteArray,
    privateKey: ByteArray,
    keyId: String? = null,
    publicKeyFormat: EdDSA.PublicKey.Format = EdDSA.PublicKey.Format.RAW,
    privateKeyFormat: EdDSA.PrivateKey.Format = EdDSA.PrivateKey.Format.RAW,
    cryptographyProvider: CryptographyProvider = CryptographyProvider.Default,
): SigningKey.SigningKeyPair {
    val eddsaPublicKey =
        cryptographyProvider
