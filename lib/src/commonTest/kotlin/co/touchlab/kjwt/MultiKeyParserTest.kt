...
class MultiKeyParserTest : FunSpec({
    ...
    context("multi-key JWS verification") {
        ...
        test("multiple keys same algo — correct kid selected") {
            val key1 = SigningAlgorithm.HS256.parse(
                "secret-for-k1-at-least-256-bits-long".encodeToByteArray(), keyId = "k1"
            )
            val key2 = SigningAlgorithm.HS256.parse(
                "secret-for-k2-at-least-256-bits-long".encodeToByteArray(), keyId = "k2"
            )
